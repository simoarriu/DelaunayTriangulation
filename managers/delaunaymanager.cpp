                ...
                cg3::Point2Dd(BOUNDINGBOX, BOUNDINGBOX))    
{
    ...
    //First, set the bounding triangle
    delaunayTriangulation.boundingTriangle(BT_P1,BT_P2,BT_P3);
    //Import the vector of triangles to draw the triangulation
    delaunayDrawable.importTriangle(delaunayTriangulation.getTriangles());

    //We the program starts, the bounding triangle and the voronoi diagram are set to false
    delaunayDrawable.visibilityBoundingTriangle(false);
    voronoiDiagram.setState(false);

    //Push the objects and update the canvas
    mainWindow.pushObj(&delaunayDrawable, "Delaunay Triangulation");
...
DelaunayManager::~DelaunayManager() {
    ...
    //Clear the triangulation and delete the pointers when the program stops and delete the drawables
    delaunayTriangulation.clearTriangulation();
    mainWindow.deleteObj(&delaunayDrawable);
...
void DelaunayManager::computeDelaunayTriangulation(const std::vector<cg3::Point2Dd>& inputPoints) {
    ...
    //Shuffle the vector of points to compute the randomized incremental algorithm
    std::random_shuffle(points.begin(),points.end());

    //Scan the vector of points in input and in an incremental way insert one point to the triangulation
    for (it=inputPoints.begin();it != inputPoints.end();it++)
    {
            delaunayTriangulation.fillTriangulation((*it));
    }
...
void DelaunayManager::addPointToDelaunayTriangulation(const cg3::Point2Dd& p) {
    ...
    //Take the point in input and insert it inside the triangulation
    delaunayTriangulation.fillTriangulation(p);
    //Import the vector of triangles to draw the triangulation
    delaunayDrawable.importTriangle(delaunayTriangulation.getTriangles());

    //If the voronoi diagram is on, draw with the triangulation also the voronoi diagram
    if(voronoiDiagram.getState())
        voronoiDiagram.importTriangle(delaunayTriangulation.getTriangles());
...
void DelaunayManager::clearDelaunayTriangulation() {
    ...
    //Clear the data structures and delte the pointers
    delaunayTriangulation.clearTriangulation();
...
void DelaunayManager::drawDelaunayTriangulation() {
    ...
    //Update the triangulation when an input vector of points is take from file
    delaunayDrawable.importTriangle(delaunayTriangulation.getTriangles());
...
void DelaunayManager::eraseDrawnDelaunayTriangulation() {
    ...
    //Clear the vector of triangles belonging to the drawable of voronoi and delaunay
    delaunayDrawable.clearTrianglesDrawable();
    voronoiDiagram.clearDiagramDrawable();
    //set the state of voronoi off
    voronoiDiagram.setState(false);
    //Import the bounding triangle to start another triangulation
    delaunayTriangulation.boundingTriangle(BT_P1,BT_P2,BT_P3);
    delaunayDrawable.importTriangle(delaunayTriangulation.getTriangles());
...
void DelaunayManager::setVisibilityBoundingTriangle(const bool visible)
{
    ...
    delaunayDrawable.visibilityBoundingTriangle(visible);
...
void DelaunayManager::checkTriangulation() {
    ...
    //Resize the matrix
    triangles.resize(delaunayTriangulation.getTriangles().size(), 3);
    //Get the matrix useful for the validation
    triangles = delaunayTriangulation.getTrianglesValidation();
...
{
    //When the voronoi diagram button is clicked, we import the triangles that compose
    //the triangulation, set the state of the voronoi diagram to true and update the canvas
    voronoiDiagram.importTriangle(delaunayTriangulation.getTriangles());
    voronoiDiagram.setState(true);
    mainWindow.updateGlCanvas();
}

void DelaunayManager::on_clearVoronoiDiagramPushButton_clicked()
{
    //When the clear voronoi diagram button is clicked, we clear the vector of triangles,
    //set the state of the voronoi diagram to false and update the canvas
    voronoiDiagram.clearDiagramDrawable();
    voronoiDiagram.setState(false);
    mainWindow.updateGlCanvas();
}
